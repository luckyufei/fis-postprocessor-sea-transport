...
module.exports = function (content, fileObj, conf) {
    var astCache, data = content;
    try {
        astCache = ast.getAst(data);
    } catch (e) {
        fis.log.error('js parse error ');
        fis.log.error(e.message + ' [ line:' + e.line + ', col:' + e.col + ', pos:' + e.pos + ' ]');
    }

    var meta = ast.parseFirst(astCache);

    if (!meta) {
        fis.log.error('found non cmd module "' + fileObj.getUrl() + '"');
        return;
    }

    if (meta.id) {
        fis.log.debug('id exists in "' + fileObj.getUrl() + '"');
    } else {
        meta.id = fileObj.getId();
    }
    ...
    if (meta.dependencyNode) {
        ...
        depsSpecified = true;
        fis.log.debug('dependencies exists in "' + fileObj.getUrl() + '"');
    } else {
        deps = parseDependencies(fileObj);
        fis.log.debug(deps.length ? 'found dependencies ' + deps : 'found no dependencies');
    }
    ...
    //TODO grunt.file.write(fileObj.dest, addOuterBoxClass(data, options));
    ...
    data = astCache.print_to_string(options.uglify);
    // TODO grunt.file.write(dest, addOuterBoxClass(data, options));
};
...
function parseDependencies(fpath) {
    var rootpath = fpath;
